...
parser = argparse.ArgumentParser(description='Chinese sensitive words variants Classification')
...
if __name__ == '__main__':
    ...
    train_iter = Dataloader(trn_data,  batch_size=int(config.batch_size), SEED=config.seed) # 训练数据，随机打乱
    # dev_iter 和 test_iter：验证和测试数据，不打乱。
    ...
    # 计算并打印数据加载耗时（get_time_dif 未在代码中定义，可能在其他模块中）
    time_dif = get_time_dif(start_time)
    print("Time usage:", time_dif)

    def experiment(tune_config):
        # 如果提供了 tune_config（超参数配置），则更新 config 中的对应参数。
        # 调用 train 函数进行训练（未在代码中定义，可能在 train_eval 模块中）。
        if tune_config:
    ...
    if args.mode == "train":
        ...
        # 调用 eval 函数（未定义，可能在 train_eval 模块中）评估模型。
