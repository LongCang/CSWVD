...
def get_all_dirty_words(base_path):
    all_dirty_words = []
    paths = ["Abbreviation.json", "CharacterDistortion.json", "LexicalLoan.json", "PhoneticReplace.json", "SemanticReplace.json","SymbolVariation.json"]
    for i in paths:
        all_dirty_words.append(get_dirty_words(base_path + i))
    return all_dirty_words

def get_variant_id(text_idx, variant_ids, dirty_words, variant_index):
        ...
        for index, token in enumerate(text_idx):
            ...
            if flag:
                for dirty_index in range(start_index, end_index):  # 对含脏词的span打上类别label
                    variant_ids[dirty_index] = variant_index
    # print(variant_ids)
    return variant_ids
       
def get_all_variant_id(pad_size, text_idx, all_dirty_words):
    variant_ids = [0 for i in range(pad_size)]
    for variant_index, dirty_words in enumerate(all_dirty_words):
        variant_ids = get_variant_id(text_idx, variant_ids, dirty_words, variant_index+1)
    return variant_ids
...
class Datasets(Dataset):
    ...
    def preprocess_data(self):
        ...
        for row in tqdm(self.data_file):
            ...
            row['variant_ids'] = get_all_variant_id(self.max_tok_len, row['text_idx'], all_dirty_words)
    ...
    def __getitem__(self, idx, corpus=None):
        ...
        sample = {
                    # For BERT
                    'text_idx': row['text_idx'], 'text_ids': row['text_ids'], 'text_mask': row['text_mask'], 'variant_ids': row['variant_ids'],
                    # For GloVe
                    # 'text_token_ids': row["text_token_ids"]
                    'variant': row["variant_one_hot"]
            # , 'variant_type': row["variant_type_one_hot"], 'expression': row["expression_one_hot"], 'target': row["target"]
                }
        # For label
...
def to_tensor(batch):
    ...
    args['variant_ids'] = torch.tensor([b['variant_ids'] for b in batch])
    ...
    args['variant'] = torch.tensor([b['variant'] for b in batch])
    # args['variant_type'] = torch.tensor([b['variant_type'] for b in batch])
